...
navLinkBtn.forEach((elem, index) => {
  elem.addEventListener("click", function (ev) {
    let btnElem = ev.target.innerText;
    switch (btnElem) {
      case "Home":
...
checkOutBtn.addEventListener("click", () => {
  ...
  document.getElementById("checkout-items").textContent = `🛒` + cart.getCounts();

  cart.items.forEach((dessert) => {
    checkout.innerHTML += `
    <p class="receipt">${dessert.name} <span>${dessert.price}</span></p>`;
  });

  document.getElementById('checkout-total').textContent = cart.calculateTotal().toFixed(2)
});
